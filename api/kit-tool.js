...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (q.action === 'pubs') {
      ...
      return res.status(200).json({ publications: ((data.data && data.data.publications && data.data.publications.nodes) || []).map(n => ({ id: n.id, name: n.name })) });
    }
    ...
    if (q.action === 'publish' && req.method === 'POST') {
      ...
      for (const pid of body.product_ids) {
        const data = await gql(headers, `mutation($id:ID!,$input:[PublicationInput!]!){ publishablePublish(id:$id, input:$input){ userErrors{ message } } }`, { id: gidProduct(pid), input });
        const ue = (data.data && data.data.publishablePublish && data.data.publishablePublish.userErrors) || data.errors || [];
        out.push({ product_id: pid, ok: ue.length === 0, channels: input.length, errors: ue.length ? ue : undefined });
      }
