...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (q.action === 'prodcols') {
      const data = await gql(headers, `query($id:ID!){ product(id:$id){ title collections(first:60){ nodes{ id title handle ruleSet{ rules{ column } } } } } }`, { id: gidProduct(q.id) });
      const p = data.data && data.data.product;
      if (!p) return res.status(404).json({ error: 'not found', raw: data });
      return res.status(200).json({ product: p.title, collections: p.collections.nodes.map(c => ({ id: c.id.split('/').pop(), title: c.title, handle: c.handle, type: c.ruleSet ? 'smart' : 'manual' })) });
    }

    // lister les publications (canaux de vente)
    if (q.action === 'pubs') {
      const data = await gql(headers, `query{ publications(first:25){ nodes{ id name } } }`, {});
      const nodes = (data.data && data.data.publications && data.data.publications.nodes) || [];
      return res.status(200).json({ publications: nodes.map(n => ({ id: n.id, name: n.name })), raw: data.errors || null });
    }

    // publier des produits sur des canaux (toutes les publications si non précisé)
    if (q.action === 'publish' && req.method === 'POST') {
      ...
      if (!pubIds || !pubIds.length) {
        ...
        pubIds = ((pd.data && pd.data.publications && pd.data.publications.nodes) || []).map(n => n.id);
      }
      const input = pubIds.map(id => ({ publicationId: id }));
      const out = [];
      for (const pid of body.product_ids) {
        const data = await gql(headers, `mutation($id:ID!,$input:[PublicationInput!]!){ publishablePublish(id:$id, input:$input){ userErrors{ field message } } }`, { id: gidProduct(pid), input });
        const ue = (data.data && data.data.publishablePublish && data.data.publishablePublish.userErrors) || data.errors || [];
        out.push({ product_id: pid, ok: ue.length === 0, errors: ue.length ? ue : undefined, channels: input.length });
      }
    ...
    if (q.action === 'create' && req.method === 'POST') {
      ...
      for (const spec of body.products) {
        ...
        if (spec.colors && spec.colors.length) {
          options = [{ name: 'Couleur' }, { name: spec.option_name || 'Taille' }];
          variants = [];
          for (const c of spec.colors) for (const s of spec.sizes) variants.push(Object.assign({ option1: c, option2: s }, baseV));
        } else {
          options = [{ name: spec.option_name || 'Taille' }];
          variants = spec.sizes.map(s => Object.assign({ option1: s }, baseV));
        }
