...
async function scanOrdersStock(req, res) {
  try {
    ...
    for (let off = 0; ; off += 1000) {
      const r = await fetch(`${SB_URL}/rest/v1/stock?select=id,product,size,qty,status&qty=gt.0&status=in.(retour,stock,flocage_joueur)&deleted_at=is.null&limit=1000&offset=${off}`, { headers: supabaseHeaders(true) });
      if (!r.ok) break;
...
async function invList(req, res) {
  try {
    const all = [];
    for (let off = 0; ; off += 1000) {
      const r = await fetch(`${SB_URL}/rest/v1/stock?select=id,product,size,qty,counted_qty,counted_at,image,status&status=in.(retour,stock,flocage_joueur)&deleted_at=is.null&order=product.asc,size.asc&limit=1000&offset=${off}`, { headers: supabaseHeaders(true) });
      if (!r.ok) throw new Error(await r.text());
      const rows = await r.json();
      all.push(...rows);
      if (rows.length < 1000) break;
...
async function invReset(req, res) {
  try {
    const r = await fetch(`${SB_URL}/rest/v1/stock?status=in.(retour,stock,flocage_joueur)`, { method: 'PATCH', headers: { ...supabaseHeaders(true), Prefer: 'return=minimal' }, body: JSON.stringify({ counted_qty: 0, counted_at: null }) });
    if (!r.ok) throw new Error(await r.text());
