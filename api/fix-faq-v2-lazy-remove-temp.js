// TEMPORARY — FAQ v2 (cadre rouge, espaces réduits, V3 tokens) + suppression script lazy custom
// DELETE AFTER USE
...
const THEME_ID = '160850116827'; // "claude working ..." — LIVE

// ── FAQ v2 : cadre rouge + tokens V3 + espacement réduit ────────────────
...
{% stylesheet %}
  ...
  @media (max-width: 640px) {
    .touni-faq { padding: 24px 16px 20px; }
    .touni-faq__header { margin-bottom: 20px; }
  }
{% endstylesheet %}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── 2. Read theme.liquid and remove ALL custom lazy scripts ──────────
    ...
    if (!theme) {
      results.lazy_removal = { ok: false, error: 'Could not read theme.liquid' };
    } else {
      let removedCount = 0;

      // Remove scripts with data-touni-lazy attribute
      const attrRe = /<script\s+data-touni-lazy="[^"]*">[\s\S]*?<\/script>/g;
      const afterAttr = theme.replace(attrRe, () => { removedCount++; return ''; });
      if (afterAttr !== theme) {
        theme = afterAttr;
      }

      // Also remove any remaining IIFE with fixLazySizes or tryUnveil (no data attr)
      const fnRe = /<script>[\s\S]{0,200}function (?:fixLazySizes|tryUnveil)\(\)[\s\S]*?<\/script>/g;
      const afterFn = theme.replace(fnRe, () => { removedCount++; return ''; });
      if (afterFn !== theme) {
        theme = afterFn;
      }
      ...
      results.lazy_removal = {
        ok: pushR.ok,
        scripts_removed: removedCount,
        updated_at: pushD.asset?.updated_at,
        // Confirm no lazy script remains
        custom_script_still_present: theme.includes('data-touni-lazy') || theme.includes('fixLazySizes') || theme.includes('tryUnveil'),
      };
    }

    res.json({ ok: true, theme_id: THEME_ID, results });
  } catch (e) {
